...
# Data from the intensity table for 0←1 transition (row "1 ← 0")
intensity_data = {
    'HCl': 0.6578,
    'HF': 0.682,
    'HBr': 0.7061,
...
def create_franck_condon_plot():
    ...
    #fig.suptitle('Franck-Condon Experiment: 0←1 Transition Intensities', 
    ...
    ax1.set_title('0←1 Vibrational Transition Intensities', fontsize=14, pad=20)
    ...
    ax1.axhspan(0.7, 0.8, alpha=0.1, color='green', label='High Intensity')
    ax1.axhspan(0.6, 0.7, alpha=0.1, color='orange', label='Medium Intensity')
    ax1.axhspan(0.35, 0.6, alpha=0.1, color='red', label='Low Intensity')
...
def print_analysis():
    ...
    print("="*60)
    print("FRANCK-CONDON 0←1 TRANSITION INTENSITY ANALYSIS")
    print("="*60)
...
def save_data_to_csv():
    ...
    df = pd.DataFrame(list(intensity_data.items()), 
                      columns=['Molecule', 'Intensity_0_to_1'])
    
    # Add additional calculated columns
    max_intensity = df['Intensity_0_to_1'].max()
    df['Relative_Percentage'] = (df['Intensity_0_to_1'] / max_intensity * 100).round(1)
    df['Molecular_Type'] = df['Molecule'].apply(classify_molecule)
    
    # Sort by intensity
    df = df.sort_values('Intensity_0_to_1', ascending=False)
    
    # Save to CSV
    df.to_csv('franck_condon_intensities.csv', index=False)
    print(f"\nData saved to 'franck_condon_intensities.csv'")
    return df
...
def create_comparison_plot():
    ...
    fig.suptitle('Detailed Franck-Condon Analysis', fontsize=16, fontweight='bold')
    ...
    ax3 = axes[1, 0]
    ax3.hist(df['Intensity'], bins=8, alpha=0.7, color='skyblue', 
    ...
    ax4.set_title('Intensity by Molecular Type')
    ax4.legend()
...
if __name__ == "__main__":
    ...
    # Optional: Save plots
    fig1.savefig('franck_condon_main_plot.png', dpi=300, bbox_inches='tight')
    fig2.savefig('franck_condon_analysis.png', dpi=300, bbox_inches='tight')
    print("\nPlots saved as 'franck_condon_main_plot.png' and 'franck_condon_analysis.png'")
    ...
    try:
        ...
        fig_interactive.update_layout(
            title='Interactive Franck-Condon 0←1 Transition Intensities',
...
def find_similar_intensities(target_intensity, tolerance=0.05):
...
def run_additional_analysis():
    ...
    print(f"\nMolecules similar to CO (0.752 ± 0.05):")
    similar_to_co = find_similar_intensities(0.752, 0.05)
    for mol, intensity in similar_to_co:
        print(f"  {mol}: {intensity:.4f}")

# Uncomment the line below to run additional analysis
